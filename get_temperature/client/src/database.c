...
int create_database_table(void)
{
    ...
    snprintf(sql,sizeof(sql) ,"CREATE TABLE if not exists temperature(SN CHAR(10),DATIME CHAR(50),TEMPERATURE CHAR(20));");
...
int data_insert(pack_info_t pack_info, char *table_name)
{
	...
	printf("Last data insert table successfully: %s, %s, %f\n" ,\
...
int check_table_data(char *table_name)
{
	...
	snprintf(sql,sizeof(sql) ,"select * from %s ORDER BY ROWID ASC limit 1", table_name);//查询第一行数据
...
int get_table_data(pack_info_t *pack_info,  char *table_name)
{
	...
	memset(sql, 0, sizeof(sql));
	snprintf(sql,sizeof(sql), "select * from %s ORDER BY ROWID ASC limit 1", table_name);
